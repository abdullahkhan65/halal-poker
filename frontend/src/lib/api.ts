...
export const api = {
  ...
  users: {
    ...
    updateMe: (data: { name?: string; avatarUrl?: string; avatarStyle?: string }) =>
  ...
  tournaments: {
    ...
    create: (name: string) =>
      request<Tournament>('/tournaments', { method: 'POST', body: JSON.stringify({ name }) }),
    ...
    reportWinner: (matchId: string, winnerId: string) =>
      request<Tournament>(`/tournaments/matches/${matchId}/winner`, {
        method: 'PATCH',
